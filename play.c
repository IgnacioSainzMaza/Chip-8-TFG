...
#define SCREEN_ROWS (DP_ROWS * PIXEL_SIZE)
#define SCREEN_COLS (DP_COLUMNS * PIXEL_SIZE)
unsigned char screen[SCREEN_ROWS][SCREEN_COLS][3];

extern uint8_t key[KEY_SIZE];
extern uint8_t dp[DP_ROWS][DP_COLUMNS];
...
void draw() {
    ...
    for (row = 0; row < DP_ROWS; row++) {
        for (col = 0; col < DP_COLUMNS; col++) {
            paint_cell(row, col, dp[row][col] ? WHITE : BLACK);
        }
    }
